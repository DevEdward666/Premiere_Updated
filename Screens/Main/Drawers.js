...
import { action_set_device_token, action_set_token } from "../../Services/Actions/Default_Actions";
import { useDispatch, useSelector } from "react-redux";
import MainDrawer from "./MainDrawer";
import { View } from "react-native";
import { IconButton } from "react-native-paper";
import { action_passbase_get_single_info } from "../../Services/Actions/PassbaseActions";
import { action_GET_Docs, action_GET_Profileimage, action_setqr } from "../../Services/Actions/Users_Actions";
    ...
    const insertdevicetoken = () => {
      if (mounted) {
        if (user_details?.prem_id!=="" && fcmtoken?.token !== undefined) {
          dispatch(action_set_device_token(user_details?.prem_id?.toString(), fcmtoken?.token, getdeviceid));
        }
      }
    };
